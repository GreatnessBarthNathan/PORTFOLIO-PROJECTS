...
function AppProvider ({children}) {
    ...
    function handleSubmit (e) {
      ...
      if (title && note && !editFlag) {
        dispatch({signal: 'NEW NOTE', title: title, note: note, id: Math.random()})
        setTitle('')
        setNote('')
      }
      if (title && note && editFlag) {
        dispatch({signal: 'EDIT NOTE', editTitle: title, editNote: note, id: editID})
        setEditFlag(false)
        setEditID(null)
        setTitle('')
        setNote('')
...
useEffect(()=> {
   localStorage.setItem('note', JSON.stringify(state.note))
    ...
    return <context.Provider value={{title, note, handleSubmit, setTitle, setNote, state, deleteNote, editNote, editFlag, showModal, setShowModal, viewModal}}>
        {children}
    </context.Provider>
}
